...
export const useDetectionStore = create<DetectionState>((set) => ({
  ...
  addDetection: (detection) =>
    set((state) => {
      ...
      // Newest first — during a live sermon, recency matters most
  ...
  addDetections: (incoming) =>
    set((state) => {
      ...
      // Incoming (newest) first, then existing — preserves recency order
      const result: DetectionResult[] = []
      for (const d of incoming) {
        if (map.has(d.verse_ref)) {
          result.push(map.get(d.verse_ref)!)
          map.delete(d.verse_ref)
        }
      }
      for (const d of state.detections) {
        if (map.has(d.verse_ref)) {
      ...
      return { detections: result.slice(0, 50) }
    }),
